...
export default function HRBranchesPage() {
  ...
  const [addType, setAddType] = useState<'department' | 'branch'>('branch') // Default to branch here
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl sm:text-3xl font-bold text-foreground">Branches</h2>
            <p className="text-muted-foreground text-sm mt-0.5">Manage office locations & departments</p>
          </div>
        </div>
          ...
          <DialogContent showCloseButton={false} className="bg-white border-0 max-w-md p-0 rounded-2xl overflow-hidden shadow-xl">
            <div className="bg-red-600 px-6 py-4 flex items-center justify-between">
              <div>
                ...
                <DialogDescription className="text-white/80 text-[10px] uppercase tracking-widest font-bold mt-1">Create a business structure</DialogDescription>
              </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="bg-white border-slate-200 p-5">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-sm text-slate-400 font-medium">Locations</p>
              <p className="text-3xl font-bold text-slate-800 mt-1">{loading ? '—' : branches.length}</p>
              <p className="text-xs text-slate-400 mt-1">Active branches</p>
            </div>
            <div className="p-2.5 rounded-lg bg-red-50"><MapPin className="w-5 h-5 text-red-600" /></div>
          </div>
        </Card>
        <Card className="bg-white border-slate-200 p-5">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">Configured groups</p>
            </div>
        ...
        <Card className="bg-white border-slate-200 p-5">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-sm text-slate-400 font-medium">Total Employees</p>
              <p className="text-3xl font-bold text-slate-800 mt-1">{loading ? '—' : totalEmployees}</p>
              <p className="text-xs text-slate-400 mt-1">Active workforce</p>
            </div>
      ...
      {/* ── Branches Cards ── */}
      <div>
        <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-3 whitespace-nowrap">Manage Office Branches</h3>
        {loading ? (
            ...
            {branches.map(branch => {
              ...
              return (
                <div key={branch.id} className="bg-white border border-slate-200 rounded-xl p-4 flex items-center gap-3 hover:shadow-md transition-all group">
                  <div className="w-10 h-10 rounded-lg bg-red-600 flex items-center justify-center text-white text-xs font-black shrink-0 shadow-lg shadow-red-600/20">
                    <MapPin className="w-4 h-4" />
                  </div>
      ...
      {/* ── Search + Filter ── */}
      <Card className="bg-white border-slate-200 p-4 mt-8">
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex-1 relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
            <Input
              placeholder="Search departments within branches..."
      ...
      {/* ── Departments List in Branches view ── */}
