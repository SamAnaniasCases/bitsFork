    ...
    const fetchData = async () => {
      try {
        ...
        if (empData.success) {
          ...
          // Compute department counts — prefer Department.name relation, fallback to string field
          const dCounts: Record<string, number> = {}
          const bCounts: Record<string, number> = {}
          activeEmps.forEach((e: any) => {
            const dept = e.Department?.name || e.department || null
            if (dept) dCounts[dept] = (dCounts[dept] || 0) + 1
